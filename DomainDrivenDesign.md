...
50. **开放主机服务**：当一个子系统要和其他许多子系统集成时，为每一个子系统定制一个转换器会使整个团队陷入困境。解决这一个问题的方法是，将外部子系统看作是服务提供者。定义一个能以服务的形式访问子系统的协议。开放它，使得所有需要和你集成的人都获取到。然后优化和扩展这个协议，使其可以处理新的集成需求，但某团队有特殊需求时除外。对于特殊的需求，使用一个一次性的转换器增加协议，从而使得共享的协议保持简洁和精干。

51. 精炼：思路是定义一个代表领域本质的核心域（Core Domain）。精炼过程的副产品是组合领域中其他部分的普通子域（Generic Subdomain）。当处理一个大的模型时，应该试图将本质概念从普通概念中分离出来。将所有的才能都投入到核心域上，不停地努力，找到一种深刻的模型，做一个灵活的设计以满足系统的远景。根据其他部分对提炼后核心的支持，判断投资。

52. 标识出项目中不是推动因素相关子域。找出那些子域的普通模型，并将他们放在不同的模型中，不要在这些模型中留下什么特殊的标记。
